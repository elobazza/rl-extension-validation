...
globals [ave-reward episode r-episode]
...
to setup
  ...
  ask patches with [pycor = 0 and pxcor > 0 and pxcor < 5][set pcolor blue set reward -100 set plabel reward]
  ...
  ask Walkers [
    learningextension:state-def ["xcor" "ycor"]
    (learningextension:actions [goUp] [goDown] [goLeft] [goRight])
...
to go
  while [episode <= 100][
    ask Walkers [set xcor 0 set ycor 0]
    ask Walkers [
      while [[pcolor] of patch-here = grey or [pcolor] of patch-here = red + 1][
